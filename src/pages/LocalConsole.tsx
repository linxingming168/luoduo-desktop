  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        {systemInfo.platform && (
          <div className="text-xs px-3 py-1.5 rounded-full bg-gray-50 dark:bg-gray-900/20 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-800">
      ...
      {!isElectron && (
        <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-900/20 border border-gray-200 dark:border-gray-800 rounded-xl flex items-center gap-3 text-sm text-gray-700 dark:text-gray-300">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
            ...
            <button onClick={handleGenerate} disabled={!goal.trim() || loading || !isElectron}
              className="mt-3 px-5 py-2 rounded-xl bg-gray-900 hover:bg-gray-900 disabled:bg-gray-300 text-white text-sm flex items-center gap-2 transition-colors">
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                  {approved ? <ShieldCheck className="w-4 h-4 text-gray-500" /> : <ShieldAlert className="w-4 h-4 text-gray-500" />}
                  生成命令
                </h2>
                {!approved ? (
                  <button onClick={() => setApproved(true)}
                    className="text-xs px-3 py-1 rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-900/30 dark:text-gray-300 transition-colors">
                    我确认要执行
                  </button>
                ) : (
                  <span className="text-xs text-gray-500 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> 已确认</span>
                )}
              </div>
              <pre className="p-3 bg-gray-900 dark:bg-black text-gray-400 text-xs rounded-xl overflow-x-auto font-mono whitespace-pre-wrap">
                {command || '(空)'}
              </pre>
          ...
          {/* 执行结果 */}
          {result && (
            <div className={`rounded-xl p-4 border ${result.ok ? 'bg-gray-50 dark:bg-gray-900/10 border-gray-200 dark:border-gray-800' : 'bg-gray-50 dark:bg-gray-900/10 border-gray-200 dark:border-gray-800'}`}>
              <div className="flex items-center gap-2 mb-2 text-sm font-semibold">
                {result.ok ? <><CheckCircle className="w-4 h-4 text-gray-500" /> 执行成功</> : <><AlertTriangle className="w-4 h-4 text-gray-500" /> 执行异常</>}
              </div>
              {result.stdout && (
              ...
              {(result.stderr || result.error) && (
                <pre className="text-xs text-gray-600 bg-gray-50 dark:bg-gray-900/20 p-3 rounded-lg mt-2 max-h-32 overflow-y-auto font-mono">{result.stderr || result.error}</pre>
              )}
            </div>
          )}
        ...
        <div className="space-y-3">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3 flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-gray-500" /> 安全机制
            </h2>
