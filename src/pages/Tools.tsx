...
import { Wrench, Play, Search, Globe, FileText, Send, Code, Terminal, ExternalLink, CheckCircle, XCircle, Loader2, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function Tools() {
  ...
  useEffect(() => {
    const key = getApiKey();
    if (!key) { setError('请在设置中配置 API Key'); return; }
    fetch(`https://tyb.ap100168.com/api/tools/list?api_key=${key}`)
      .then(r => r.json())
      .then(d => { setTools(d.data || []); setError(''); })
      .catch(e => setError('加载工具列表失败: ' + e.message));
  }, []);
