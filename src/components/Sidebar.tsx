...
import { Bot, MessageSquare, BookOpen, Wrench, Settings, LayoutDashboard, Target, FolderOpen, Monitor, Radio, Zap, Lightbulb, ChevronDown, ChevronRight, Coins, Gift, ExternalLink, Sparkles, GitBranch, Puzzle, Key, Smartphone } from 'lucide-react';
...
const moreNavItems = [
  ...
  { id: 'channels', label: '发布渠道', icon: Smartphone },
...
export default function Sidebar({ currentPage, onNavigate, online }: Props) {
  ...
  return (
    <div className="w-60 flex-shrink-0 bg-[--color-sidebar] text-gray-700 flex flex-col h-full border-r border-[#ebebeb]">
      {/* Logo */}
      ...
      <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
        ...
        <div className="pt-2">
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors uppercase tracking-wider"
          >
            {moreOpen ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
            更多
          </button>
          {moreOpen && (
            <div className="mt-1 space-y-1 pl-2 border-l border-[#ebebeb] ml-1.5">
      ...
      <div className="px-3 py-3 border-t border-[#ebebeb]">
        <div className="bg-gray-50 border border-[#ebebeb] rounded-xl p-3">
          <div className="flex items-center gap-2 mb-2">
            <Coins className="w-4 h-4 text-gray-500" />
            <span className="text-xs font-semibold text-gray-700">落朵权益值</span>
          </div>
