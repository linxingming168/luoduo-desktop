...
import { Bot, MessageSquare, BookOpen, Wrench, Settings, LayoutDashboard, Target, FolderOpen, Monitor, Radio, Zap, Lightbulb, ChevronDown, ChevronRight, Coins, Gift, ExternalLink, Sparkles } from 'lucide-react';
...
export default function Sidebar({ currentPage, onNavigate, online, darkMode, onToggleDark }: Props) {
  ...
  const navButton = (id: string, label: string, icon: any, cls = '') => (
    <button
      key={id}
      onClick={() => onNavigate(id)}
      className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors ${cls} ${
        currentPage === id
  ...
  return (
    <div className="w-60 flex-shrink-0 bg-[--color-sidebar] text-gray-200 flex flex-col h-full">
      ...
      <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
        {mainNavItems.map(({ id, label, icon: Icon }) => navButton(id, label, <Icon className="w-4 h-4 flex-shrink-0" />))}
        ...
        <div className="pt-2">
          ...
          {moreOpen && (
            <div className="mt-1 space-y-1 pl-2 border-l border-gray-700/30 ml-1.5">
              {moreNavItems.map(({ id, label, icon: Icon }) => navButton(id, label, <Icon className="w-4 h-4 flex-shrink-0" />))}
            </div>
          )}
        </div>
      </nav>
